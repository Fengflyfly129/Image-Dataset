...
#数据集图片爬取函数
def craw_single_class(keyword, DOWNLOAD_NUM=200):#keyword类别，DOWNLOAD_NUM爬取数量
...
dataset_path = r'D:\PyCharm\数据增强\dataset'
#删除非三通道的图片
def delete_extra_img(dataset_path):
    for category in tqdm(os.listdir(dataset_path)):
        for file in os.listdir(os.path.join(dataset_path, category)):
            file_path = os.path.join(dataset_path, category, file)
...
craw_single_class('柚子', DOWNLOAD_NUM = 10)
delete_extra_img(dataset_path)
